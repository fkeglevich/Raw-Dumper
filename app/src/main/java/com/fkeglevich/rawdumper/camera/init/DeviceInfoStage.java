...
public class DeviceInfoStage extends CameraInitStage
{
    DeviceInfoStage(CameraContextBuilder builder, CameraInitStage nextStage)
    {
        super(builder, nextStage);
    }

    @Override
    public void executeStage(final AsyncOperation<CameraContext> callback, final AsyncOperation<MessageException> exception)
    {
        IOThread.getIOAccess().loadDeviceInfo(new AsyncOperation<DeviceInfo>()
        {
            ...
            {
                builder.buildDeviceInfo(argument);

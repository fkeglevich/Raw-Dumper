...
public class FinalStage extends CameraInitStage
{
    private final AsyncOperationPoster mainThreadPoster;

    FinalStage(CameraContextBuilder builder)
    {
        super(builder, null);
        mainThreadPoster = new AsyncOperationPoster(Looper.getMainLooper());
    }

    @Override
    public void executeStage(AsyncOperation<CameraContext> callback, AsyncOperation<MessageException> exception)
    {
        mainThreadPoster.enqueueOperation(callback, builder.build());
    }
}
